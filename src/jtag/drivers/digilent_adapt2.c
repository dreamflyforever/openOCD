...
COMMAND_HANDLER(adapt2_select_device_cmd)
{
    ...
    if(!DjtgPutTmsTdiBits(hif, rgbSetup, NULL, 9, 0)) {
    ...
    do {
        ...
        if(!DjtgGetTdoBits(hif, 0, 0, rgbTdo, 32, 0)) {
...
static int bb_execute_tms(struct jtag_command *cmd)
{
	...
	BYTE * bits = (BYTE *)cmd->cmd.tms->bits;
...
static void bb_stableclocks(int num_cycles)
{
	BYTE  tms = (tap_get_state() == TAP_RESET ? 1 : 0);
...
//    LOG_INFO("------------\nscan_size = %d", scan_size);
//    LOG_INFO("type = %d | %d %d", type,  type == SCAN_OUT, type == SCAN_IN);
	if (!((!ir_scan &&
...
//		bitbang_interface->write(0, tms, tdi);
        snd[bit_cnt / 4] |= (tdi << ((bit_cnt % 4) * 2));
        snd[bit_cnt / 4] |= (tms << ((bit_cnt % 4) * 2 + 1));
...
//		}
    ...
    if(!DjtgPutTmsTdiBits(hif, snd, rcv, scan_size, 0)) {
        LOG_INFO("DjtgPutTmsTdiBits failed");
        pre_exit();
    }

    if (type != SCAN_OUT)
        for (bit_cnt = 0; bit_cnt < scan_size; bit_cnt+=8) {
    		int bytec = bit_cnt / 8;
//            LOG_INFO("%02x|%02x", snd[bytec] & 0xaa, rcv[bytec]);
            buffer[bytec] = rcv[bytec];
        }
